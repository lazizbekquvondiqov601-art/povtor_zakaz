...
def register_handlers(dp, bot: Bot, STAT_CACHE, OBR_CACHE):

    @dp.message(lambda m: m.text == "🔍 Supplier Tahlil" and m.from_user.id == db_manager.config.SUPER_ADMIN_ID)
    ...
    @dp.message(SupplierSearch.waiting_query)
    ...
    @dp.callback_query(lambda c: c.data.startswith("supSel_"))
    async def supplier_selected(callback: CallbackQuery, state: FSMContext):
        ...
        msg = await callback.message.edit_text(
            f"⏳ <b>{supplier_name}</b> yuklanmoqda..."
        )

        sotuv, qoldiq, start_date, end_date = await asyncio.to_thread(
            get_supplier_sales_report, supplier_name
        )
        ...
        if not sotuv and not qoldiq:
            await msg.edit_text(
                f"⚠️ <b>{supplier_name}</b> uchun sotuv ma'lumoti topilmadi.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
            )
            return

        chart_buf = await asyncio.to_thread(
            generate_supplier_table_image, supplier_name, sotuv, qoldiq, start_date, end_date
        )
        ...
        if chart_buf:
            ...
            await bot.send_photo(
                ...
                photo=photo,
                caption=f"👤 <b>{supplier_name}</b>\n📅 Oy boshidan: <b>{start_date} — {end_date}</b>",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
            )
        else:
            await msg.edit_text(
                f"❌ Jadval yaratishda xatolik.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
            )

    @dp.callback_query(lambda c: c.data.startswith("supOBR_"))
    async def supplier_obr_click(callback: CallbackQuery):
        ...
        msg = await bot.send_message(
            ...
            f"⏳ <b>{supplier_name}</b> — OBR hisoblanmoqda...\n10-20 soniya kuting."
        )
        ...
        # Faqat exact match — supplier_name d_mahsulotlar dagi to'liq nom
        ...
        for c in cats:
            ...
            kb.append([InlineKeyboardButton(
                text=f"📁 {c}",
                callback_data=f"obrCat_{session_id}_{cat_id}"
            )])

        kb.append([InlineKeyboardButton(text="❌ Yopish", callback_data="del_msg")])

        await msg.edit_text(
            f"📊 <b>{supplier_name} — OBR ZAKAZ</b>\n\nKategoriyani tanlang:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
        )