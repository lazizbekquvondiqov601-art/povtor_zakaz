...
# ============================================================
# 1. BAZADA f_importlar JADVALINI TAYYORLASH
# ============================================================
...
def get_imported_summary_by_dax(engine, start_date_str: str, end_date_str: str) -> str:
    try:
        ...
        msg = f"📥 <b>KELGAN TOVARLAR TAHLILI</b>\n📅 <b>{start_date_str} — {end_date_str}</b>\n"

        jami_oddiy = jami_aksiya = 0
        if oddiy:
            msg += "\n━━━━━━━━━━━━━━━━\n🏢 <b>ASOSIY TOVARLAR</b>\n"
            for pol_k, items in sorted(oddiy.items()):
                items = dict(sorted(items.items(), key=lambda x: x[1], reverse=True))
                jami = sum(items.values()); jami_oddiy += jami
        ...
        if aksiya:
            ...
            for pol_k, items in sorted(aksiya.items()):
                items = dict(sorted(items.items(), key=lambda x: x[1], reverse=True))
                jami = sum(items.values()); jami_aksiya += jami
                msg += f"\n<b>{pol_k}</b> — jami: <b>{int(jami)} dona</b>\n<pre>"
                for dax, qty in items.items():
                    dax_short = dax[:28] if len(dax) > 28 else dax
                    msg += f"  {dax_short:<28} {int(qty):>5} dona\n"
        ...
        msg += f"\n━━━━━━━━━━━━━━━━\n🏢 Asosiy : <b>{int(jami_oddiy)} dona</b>\n🎁 Aksiya  : <b>{int(jami_aksiya)} dona</b>\n🚛 <b>JAMI: {int(jami_oddiy + jami_aksiya)} dona</b>"
        return msg
