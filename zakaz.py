    ...
    try:
        ...
        d_mahsulotlar = pd.read_sql(
            '''SELECT "product_id", "Артикул", "Наименование", "Категория", "Подкатегория",
                      "Материал", "Вид", "Цена продажи", "Размер сетка"
               FROM d_mahsulotlar''',
            engine
        )
        ...
        f_sotuvlar = f_sotuvlar.drop_duplicates()
        f_qoldiqlar = f_qoldiqlar.drop_duplicates()
        ...
        GROUP_KEYS = ['Категория', 'Подкатегория', 'Наименование', 'Real_Sotuv_Segmenti', 'Размер сетка', 'Материал2', 'Вид2']
        ...
        # B) HOZIRGI QOLDIQ
        max_dates_per_shop = f_qoldiqlar.groupby('Магазин')['Дата'].transform('max')
        hoz_q = f_qoldiqlar[f_qoldiqlar['Дата'] == max_dates_per_shop]
        ...
        def apply_dax(row):
            ...
            # TargetDays & Modifier
            if obr >= 2.0:   target_days, modifier = 22, 1.50
            elif obr >= 1.5: target_days, modifier = 20, 1.35
            elif obr >= 1.2: target_days, modifier = 18, 1.20
            elif obr >= 1.0: target_days, modifier = 16, 1.10
            elif obr >= 0.7: target_days, modifier = 14, 1.00
            elif obr >= 0.5: target_days, modifier = 12, 0.90
        ...
        # 5. YAKUNIY CHIQARISH VA TOZALASH
# 5. YAKUNIY CHIQARISH VA TOZALASH
        cols = [
            'Категория', 'Подкатегория', 'Наименование', 'Real_Sotuv_Segmenti', 
            'Размер сетка', 'Материал2', 'Вид2', 
            'Zakaz', 'Hozirgi_Qoldiq', 'Продано', 'OBR %'
        ]
        ...
        # 🟢 YANGI MANTIQ: Qoldiq >= 5 va Sotuv < 5 bo'lgan keraksiz qatorlarni yashirish 🟢
        # Faqat tugayotgan (qoldiq < 5) yoki yaxshi ketayotgan (sotuv >= 5) tovarlarni qoldiramiz.
        result_df = result_df[~((result_df['Hozirgi_Qoldiq'] >= 5) & (result_df['Продано'] < 5))].reset_index(drop=True)

        # Excel kabi razmer bo'yicha ketma-ketlikda tartiblaymiz! (0 bo'lsa ham chiqadi)
        result_df = result_df.sort_values(
            by=['Категория', 'Подкатегория', 'Наименование', 'Real_Sotuv_Segmenti', 'Размер сетка'],
            ascending=True
