...
def calculate_auto_zakaz(engine) -> pd.DataFrame:
    ...
    try:
        ...
        # Faqat shu oy boshidan beri ma'lumotlarni tahlil qilamiz
        oy_boshi = datetime.now(TASHKENT_TZ).replace(day=1, hour=0, minute=0, second=0, microsecond=0).replace(tzinfo=None)
        ...
        # Joriy qoldiqlar (oxirgi sana bo'yicha)
        ...
        # 5. Hisob-kitob (Logic from Legacy)
        bugun = datetime.now()
        ...
        # OBR va Kunlik sotuvga qarab target_days va modifier tanlaymiz
        conditions = [
            obr >= 2.0,
            obr >= 1.5,
            obr >= 1.2,
        ...
        modifier_vals    = [1.10, 1.00, 1.00, 0.90, 0.80, 0.75, 0.60, 0.50]
